 ...
 */
package com.kr.messaging.ui.conversation;
...
import com.kr.messaging.datamodel.binding.Binding;
import com.kr.messaging.datamodel.binding.BindingBase;
import com.kr.messaging.datamodel.binding.ImmutableBindingRef;
...
import com.kr.messaging.datamodel.data.ConversationData.SimpleConversationDataListener;
import com.kr.messaging.datamodel.data.DraftMessageData;
import com.kr.messaging.datamodel.data.DraftMessageData.CheckDraftForSendTask;
import com.kr.messaging.datamodel.data.DraftMessageData.CheckDraftTaskCallback;
import com.kr.messaging.datamodel.data.DraftMessageData.DraftMessageDataListener;
...
import com.kr.messaging.datamodel.data.PendingAttachmentData;
import com.kr.messaging.datamodel.data.SubscriptionListData.SubscriptionListEntry;
import com.kr.messaging.receiver.Test;
import com.kr.messaging.sms.MmsConfig;
import com.kr.messaging.ui.AttachmentPreview;
import com.kr.messaging.ui.BugleActionBarActivity;
import com.kr.messaging.ui.PlainTextEditText;
import com.kr.messaging.ui.conversation.ConversationInputManager.ConversationInputSink;
import com.kr.messaging.util.AccessibilityUtil;
import com.kr.messaging.util.AlertDialogHelper;
import com.kr.messaging.util.Assert;
import com.kr.messaging.util.AvatarUriUtil;
import com.kr.messaging.util.BuglePrefs;
import com.kr.messaging.util.ContentType;
import com.kr.messaging.util.LogUtil;
import com.kr.messaging.util.MediaUtil;
import com.kr.messaging.util.OsUtil;
import com.kr.messaging.util.UiUtils;
...
public class ComposeMessageView extends LinearLayout
    ...
    public interface IComposeMessageViewHost extends
            DraftMessageData.DraftMessageSubscriptionDataProvider, Serializable {
...
//    private boolean isSchedule = false;
    ...
    protected void onFinishInflate() {
        ...
        mSendButton.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(final View clickView) {
                Log.d("-------------", mBinding.getData().getMessageSubject());
        ...
        mSchedule.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                final Activity activity = (Activity) mOriginalContext;
                    ...
                    public void onDateSet(DatePickerDialog datePickerDialog, final int year, final int month, final int day) {
                        TimePickerDialog timePickerDialog = TimePickerDialog.newInstance(new TimePickerDialog.OnTimeSetListener() {
                            ...
                            public void onTimeSet(RadialPickerLayout view, int hourOfDay, int minute) {
                                ...
                                AlertDialogHelper.future = calendar;
                                AlertDialogHelper.isSchedule = true;
                            }
                        }, calendar.get(Calendar.HOUR_OF_DAY), calendar.get(Calendar.MINUTE) + 1, true);
                        timePickerDialog.show(activity.getFragmentManager(), "AAA");
                    }
                }, calendar.get(Calendar.YEAR), calendar.get(Calendar.MONTH), calendar.get(Calendar.DAY_OF_MONTH));
                datePickerDialog.show(activity.getFragmentManager(), "AAA");
            }
        });

        mEmoji = (Button) findViewById(R.id.emoji);
        final EmojiPopup emojiPopup = EmojiPopup.Builder.fromRootView(findViewById(R.id.draft_container)).build(mComposeEditText);
                    ...
                    switch (result) {
                        case CheckDraftForSendTask.RESULT_PASSED:
                            ...
                            if (message != null && message.hasContent()) {
                                playSentSound();
                                // zeus
                                if(AlertDialogHelper.isSchedule) {
                                    message.setSentTimestamp(AlertDialogHelper.future.getTimeInMillis());
                                }
//                                    isSchedule = false;
//                                    Intent service = new Intent(mOriginalContext, Test.class);
//                                    service.setAction("com.kr.messages");
//                                    PendingIntent pendingIntent = PendingIntent.getBroadcast(
//                                            mOriginalContext.getApplicationContext(), (int)(System.currentTimeMillis()), service, PendingIntent.FLAG_CANCEL_CURRENT);
//                                    AlarmManager alarmManager = (AlarmManager) mOriginalContext.getSystemService(ALARM_SERVICE);
//                                    alarmManager.setExact(AlarmManager.RTC_WAKEUP, future.getTimeInMillis(), pendingIntent);
//
//                                    final DatabaseWrapper db = DataModel.get().getDatabase();
//                                    message.setSentTimestamp(future.getTimeInMillis());
//
//                                    ContentValues contentValues = new ContentValues();
//                                    contentValues.put(DatabaseHelper.MessageColumns.CONVERSATION_ID, message.getMessageId());
//                                    contentValues.put(DatabaseHelper.MessageColumns.SENDER_PARTICIPANT_ID, message.getParticipantId());
//                                    contentValues.put(DatabaseHelper.MessageColumns.SELF_PARTICIPANT_ID, message.getSelfId());
//                                    contentValues.put(DatabaseHelper.MessageColumns.SENT_TIMESTAMP, message.getSentTimeStamp());
//                                    contentValues.put(DatabaseHelper.MessageColumns.RECEIVED_TIMESTAMP, message.getReceivedTimeStamp());
//                                    contentValues.put(DatabaseHelper.MessageColumns.SEEN, message.getMessageSeen());
//                                    contentValues.put(DatabaseHelper.MessageColumns.READ, 0);
//                                    contentValues.put(DatabaseHelper.MessageColumns.PROTOCOL, message.getProtocol());
//                                    contentValues.put(DatabaseHelper.MessageColumns.STATUS, message.getStatus());
//                                    Uri mSmsMessageUri = message.getSmsMessageUri();
//                                    final String smsMessageUri = ((mSmsMessageUri == null) ? null : mSmsMessageUri.toString());
////                                    Log.d("-----------", smsMessageUri);
//                                    contentValues.put(DatabaseHelper.MessageColumns.SMS_MESSAGE_URI, smsMessageUri);
//                                    contentValues.put(DatabaseHelper.MessageColumns.SMS_PRIORITY, message.getSmsPriority());
//                                    contentValues.put(DatabaseHelper.MessageColumns.SMS_MESSAGE_SIZE, message.getSmsMessageSize());
//                                    contentValues.put(DatabaseHelper.MessageColumns.MMS_SUBJECT, message.getMmsSubject());
//                                    contentValues.put(DatabaseHelper.MessageColumns.MMS_TRANSACTION_ID, message.getMmsTransactionId());
//                                    contentValues.put(DatabaseHelper.MessageColumns.MMS_CONTENT_LOCATION, message.getMmsContentLocation());
//                                    contentValues.put(DatabaseHelper.MessageColumns.MMS_EXPIRY, 0);
//                                    contentValues.put(DatabaseHelper.MessageColumns.RAW_TELEPHONY_STATUS, message.getRawTelephonyStatus());
//                                    contentValues.put(DatabaseHelper.MessageColumns.RETRY_START_TIMESTAMP, message.getRetryStartTimestamp());
//                                    db.insert(DatabaseHelper.SCHEDULED_MESSAGES_TABLE, null, contentValues);
//                                } else {
